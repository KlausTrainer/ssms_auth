...
%% @doc The official way to get the values set in ssms_auth's
%% environment.  Will return `undefined' if the given option is unset.
-spec get_app_env(atom()) -> term().
get_app_env(Opt) ->
    get_app_env(Opt, undefined).

%% @doc The official way to get the values set in ssms_auth's
%% environment.  Will return `Default' if the given option is unset.
