...
init_per_group(srp_integration, Config) ->
    ...
    SrpConfig = srp_2048,
    TestDbPath = code:lib_dir(ssms_auth) ++ "/test/ssms_auth_test_db.bitcask",
    application:set_env(ssms_auth, db_path, TestDbPath),
    application:set_env(ssms_auth, port, 0),
    application:set_env(ssms_auth, srp_config, SrpConfig),
    {ok, _} = ssms_auth_db:start(),
    {ok, _} = ssms_auth_cache:start([{ttl, 60000}, {name, ssms_auth_cache}]),
    {ok, _} = ssms_auth_web:start(),
